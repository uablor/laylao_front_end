<template>
  <a-modal
    ...
    ><div class="flex justify-center pt-4 w-full">
      ...
      >
        <img
          :src="baseEnv.IMAGE_URL + authme?.hotels[0]?.logo"
    ...
    <div class="flex justify-center items-center my-4 transition duration-200 hover:underline">
      ...
      <a-row :gutter="[16, 16]">
        ...
        <a-col :span="24">
          <div class="flex justify-end gap-2 mt-2">
            ...
            <BaseButton type="primary" html-type="submit" :loading="loading" :disabled="loading">
              {{ $t("save") }}
            </BaseButton>
          </div>
        </a-col>
...
watch(
  ...
  async (val) => {
    if (val) {
      await fetchUser(Number(authme.value?.id));
