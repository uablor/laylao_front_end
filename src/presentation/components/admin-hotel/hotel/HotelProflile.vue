<template>
  <div class="container mx-auto">
    <div
      v-if="loading || !selectedHotel"
      class="flex justify-center items-center h-screen"
    >
      <a-spin size="medium" :spinning="loading" />
    </div>
    ...
    <div v-else class="p-3 lg:p-6">
      ...
      <div class="flex sm:flex-row lg:flex-row w-full gap-4 h-auto">
        <div class="w-full lg:w-3/2">
          <GalleryContact
            ...
            :image-url-prefix="baseEnv.IMAGE_URL"
              ...
              >
                ...
                >
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <span>{{ selectedHotel?.email_hotel }}</span>
                  </div>
                  <div class="">
      ...
      <div class="mt-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-2 pt-4">
          ...
          >
            ...
            <span class="font-semibold">{{ selectedHotel?.floor }}</span>
          </div>
          <div
            class="flex justify-between items-center bg-gray-50 px-3 py-2 text-sm shadow-sm"
          >
          ...
          >
            ...
            <span class="font-semibold">{{ hotelDetails?.booking_total }}</span>
          </div>
        </div>
      </div>
      ...
      <div>
        <div
          v-if="selectedHotel && (selectedHotel?.room_types?.length ?? 0) > 0"
        >
          <div
...
const { hotelDetails, loading } = storeToRefs(useHotelStore());
const { fetchHotel, selectedHotel, findOneHotelDetails } = useHotel();
...
onMounted(async () => {
  if (route.params.id) {
    ...
    await fetchHotel(hotel_id.value);
    await findOneHotelDetails();
    ...
    console.log(hotel_id);
    hotel_id.value = Number(authme.value?.hotels?.[0]?.id);
    await fetchHotel(hotel_id.value);
    await findOneHotelDetails();
  }
  console.log(hotel_id);
