...
export class HotelRepositoryImpl implements IHotelRepository {
  ...
  async create(
    credentials: CreateHotelCredentialsDto
  ): Promise<HotelApiResponse> {
    const response = await this.apiClient.post<Hotel>(
  ...
  async update(
    credentials: UpdateHotelCredentialsDto
  ): Promise<HotelApiResponse> {
    const response = await this.apiClient.putOrPatch<Hotel>(
      get_endpoint_path(API_ENDPOINTS.HOTEL.BASE, credentials.id),
      credentials,
      "PATCH"
    );
    return response;
  ...
  async findOneHotelDetails(id: number): Promise<HotelApiResponseSummary> {
    const response = await this.apiClient.get<HotelSummary>(
      `${API_ENDPOINTS.HOTEL.DETAILS}/${id}`
    );
    return response;
  }
